...
const { getUsers, updateProfile, uploadAvatar, updatePushToken, getMessages, sendMessage, getMe, updatePublicKey, getPublicKey, getFriends, sendFriendRequest, acceptFriendRequest, getMyFriendRequests, getUserById, markAsRead, getNotifications, editMessage, deleteMessage, uploadMessageMedia, reactToMessage } = require('../controllers/userController');
...
const uploadMedia = multer({ storage, limits: { fileSize: 50 * 1024 * 1024 } });
...
router.route('/messages/upload-media').post(protect, uploadMedia.single('media'), uploadMessageMedia);
router.route('/messages').post(protect, sendMessage);
router.route('/messages/:messageId')
    .put(protect, editMessage)
    .delete(protect, deleteMessage);
router.route('/messages/:messageId/react').put(protect, reactToMessage);
